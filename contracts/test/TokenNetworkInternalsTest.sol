...
contract TokenNetworkInternalStorageTest is TokenNetwork {
    ...
    function updateBalanceProofDataPublic(
        ...
        return
            updateBalanceProofData(channel, participant, nonce, balance_hash);
    }
    ...
    function updateUnlockDataPublic(
        ...
        return storeUnlockData(channel_identifier, settle_input, partner);
    }
    ...
    function verifyBalanceHashDataPublic(
        ...
        return verifyBalanceHashData(to_verify_state, settle_input);
    }

    function getChannelAvailableDepositPublic(
        address participant1,
        address participant2
    ) public view returns (uint256 total_available_deposit) {
        uint256 channel_identifier = getChannelIdentifier(
            participant1,
            participant2
        );
        Channel storage channel = channels[channel_identifier];
        ...
        return
            getChannelAvailableDeposit(participant1_state, participant2_state);
    }
}
...
contract TokenNetworkSignatureTest is TokenNetwork {
    ...
    ) public view returns (address signature_address) {
        return
            recoverAddressFromBalanceProof(
                channel_identifier,
                balance_hash,
                nonce,
                additional_hash,
    ...
    function recoverAddressFromBalanceProofCounterSignaturePublic(
        ...
        return
            recoverAddressFromBalanceProofCounterSignature(
                message_type_id,
                channel_identifier,
                balance_hash,
                nonce,
...
contract TokenNetworkUtilsTest is TokenNetwork {
    ...
    function getHashAndUnlockedAmountPublic(
        bytes memory locks
    ) public view returns (bytes32, uint256) {
        return getHashAndUnlockedAmount(locks);
    }

    function getLockedAmountFromLockPublic(
        bytes memory locks,
        uint256 offset
    ) public view returns (uint256) {
    ...
    function failsafe_subtractPublic(
        uint256 a,
        uint256 b
    ...
    function failsafe_additionPublic(
        uint256 a,
        uint256 b
    ) public view returns (uint256) {
